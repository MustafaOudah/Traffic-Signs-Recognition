...
def sign_predict(image):
    model = load_model('/Users/mustafa/Desktop/myـthesis/traffic_signs_recognition/keras_model')
...
def main():
    ...
    st.set_page_config(page_title='Traffic Signs Recognition', page_icon=':car:')
    st.title('Traffic Signs Recognition')
    st.markdown("""
       Author : Mustafa Oudah Hani, Advisor : Prof. Dr. Dogu Cagdas ATILLA
        """)
    with st.expander("See list of classes"):
        st.write(list(labels_dict.values()))
    st.image('/Users/mustafa/Desktop/myـthesis/traffic_signs_recognition/app_images/Vehacle_model.png', use_column_width=True)
